...
import { SUBAGENT_RESPONSE_FORMAT_CONFIG_KEY } from "./constants.js";
...
export function createCodeInterpreterMiddleware(
  ...
  return createMiddleware({
    ...
    wrapModelCall: async (request, handler) => {
      ...
      // Fork adaptation: the fork's middleware system passes the system
      // prompt as a string on `request.systemPrompt` (see
      // src/middleware/fs.ts), not a `SystemMessage` with `.concat`.
      // Append our sections to it the same way fs.ts does.
      const currentSystemPrompt = request.systemPrompt || "";
      const appended =
        baseSystemPrompt + subagentPrompt + (cachedPtcPrompt || "");
