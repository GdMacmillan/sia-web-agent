 ...
 *
 * Ported verbatim from upstream deepagents (middleware/utils.ts). This is a
 * genome prerequisite: a name-addressable stack is what makes swap/toggle/add/
 * remove operators possible.
 */
 ...
 * `appendNew` is false.
 *
 * Ported verbatim from upstream deepagents (middleware/utils.ts).
 */
 ...
 *
 * @example
 * ```typescript * const original = new SystemMessage({ content: "You are a helpful assistant." }); *
const updated = appendToSystemMessage(original, "Always be concise."); * // Result: SystemMessage
 ...
 *
 * @example
 * ```typescript * const original = new SystemMessage({ content: "Always be concise." }); * const updated
= prependToSystemMessage(original, "You are a helpful assistant."); * // Result: SystemMessage with
