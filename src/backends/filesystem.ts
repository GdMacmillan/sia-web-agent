 ...
 * - Path validation to prevent access outside project boundary
...
export class FilesystemBackend implements BackendProtocol {
   ...
   * When virtualMode=false, resolve paths and validate they're within project boundary.
   *
   * @param key - File path (absolute, relative, or virtual when virtualMode=true)
   * @returns Resolved absolute path string
   * @throws Error if path traversal detected or path outside project boundary
   */
  private resolvePath(key: string): string {
    if (this.virtualMode) {
      // Virtual mode: existing security logic
    ...
    // This prevents access to /etc/passwd, /tmp, etc.
