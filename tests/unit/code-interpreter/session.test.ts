// Ported from upstream @langchain/quickjs (libs/providers/quickjs/src/session.test.ts).
// vitest -> @jest/globals (vi -> jest), import path rewritten. Logic verbatim.
...
describe("REPL Engine", () => {
  ...
  describe("subagent bridge", () => {
    ...
    it("should invoke dispatch with correct arguments", async () => {
      const dispatch = jest.fn<(...a: any[]) => any>().mockResolvedValue("done");
    ...
    it("should pass responseSchema to dispatch", async () => {
      const dispatch = jest.fn<(...a: any[]) => any>().mockResolvedValue({ bugs: [] });
      session = createSubagentSession(dispatch);
    ...
    it("should return structured objects as native JS values", async () => {
      ...
      const dispatch = jest.fn<(...a: any[]) => any>().mockResolvedValue(structured);
      session = createSubagentSession(dispatch);
    ...
    it("should reject when description is missing", async () => {
      const dispatch = jest.fn<(...a: any[]) => any>();
      session = createSubagentSession(dispatch);
    ...
    it("should propagate dispatch errors to guest code", async () => {
      const dispatch = jest.fn<(...a: any[]) => any>().mockRejectedValue(new Error("subagent crashed"));
      session = createSubagentSession(dispatch);
    ...
    it("should be frozen and non-writable on globalThis", async () => {
      const dispatch = jest.fn<(...a: any[]) => any>().mockResolvedValue("ok");
      session = createSubagentSession(dispatch);
    ...
    it("should gate concurrency via queue", async () => {
      ...
      const dispatch = jest.fn<(...a: any[]) => any>().mockImplementation(async () => {
        concurrentCalls++;
